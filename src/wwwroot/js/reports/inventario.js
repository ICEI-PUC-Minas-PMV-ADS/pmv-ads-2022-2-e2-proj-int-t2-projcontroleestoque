document.addEventListener("DOMContentLoaded", function () {
    ...
    const modal = $('#inventario');

    // Submit modal de Inventario

    let queryInventario = document.getElementById("inventario_submit");
    queryInventario.addEventListener("click", function () {
        initInventarioData(containter, modal);
    });
});
...
function initInventarioData(containter, modal) {
    const formData = generateFormData('inventario', [
        'realizadoPor',
        'solicitadoPor',
        'criadoStartDate',
        'criadoEndDate',
    ...
    const action = fetch('Reports/GenerateInventario', {
        method: 'POST',
        body: JSON.stringify({
            realizadoPor: formData['realizadoPor'],
            solicitadoPor: formData['solicitadoPor'],
            criadoStartDate: formData['criadoStartDate'],
            criadoEndDate: formData['criadoEndDate'],
            executadoStartDate: formData['executadoStartDate'],
            executadoEndDate: formData['executadoEndDate'],
    ...
    action
        ...
        .then((response) => processInventarioData(containter, modal, response));
}

function processInventarioData(container, modal, response) {
    ...
    const updateReports = (container, offset) => {
        ...
        action
            .then((response) => response.json())
            .then((response) => initInventarioData(container, modal, response));
    };
            ...
            createElement('table', { className: 'table table-hover table-sm', style: { backgroundColor: 'white', borderRadius: '0.25rem' } }, [
                createElement('thead', null, [
                    createElement('tr', null, [
                        createTooltipTH('Realizado Por'),
                        createTooltipTH('Solicitado Por'),
                        createTooltipTH('Data de criação'),
                        createTooltipTH('Data de execução'),
                        createTooltipTH('Numero de items')
                    ])
                ]),
                createElement('tbody', null, data['results'].map((record) => createElement('tr', null, [
                    createTooltipTD(record['realizadoPor']),
                    createTooltipTD(record['solicitadoPor']),
                    createTooltipTD(formatDate(record['dataCriacao'])),
                    createTooltipTD(formatDate(record['dataDeExecucao'])),
                    createTooltipTD(record['count']),
                ]))),
